...
sealed class StarAst<T : Ast> : AstImpl() {
    ...
    fun traverseDownTheLeft(other: StarAst<T>): StarConcat =
        if (height == other.height) baseCase(other)
            ...
            is StarConcatSingle<*> -> StarConcatSingle<T>(
                ctor(
                    listOf(instruction.node) + other.nodes.drop(1),
                    other.height
                )
            )
            ...
            is StarConcatPair<*> -> {
                ...
                if (outNodes.size <= STAR_MAX_NODES) StarConcatSingle<T>(ctor(outNodes, other.height))
                    ...
                    StarConcatPair<T>(
                        ctor(outNodes.take(halfSize), other.height),
                        ctor(outNodes.drop(halfSize), other.height)
                    )
                }
            }
        }

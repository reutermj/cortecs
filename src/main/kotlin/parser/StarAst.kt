...
sealed class StarAst<T: Ast>: AstImpl() {
    ...
    fun <S> fold(initial: S, f: (S, T) -> S): S {
        var acc = initial
        if(height == 0) for(node in nodes) acc = f(acc, node as T)
        else for(node in nodes) acc = (node as StarAst<T>).fold(acc, f)
        return acc
    }

    fun traverseDownTheLeft(other: StarAst<T>): StarConcat = if(height == other.height) baseCase(other)
    ...
    fun traverseDownTheRight(other: StarAst<T>): StarConcat = if(height == other.height) baseCase(other)
    ...
    fun baseCase(other: StarAst<T>): StarConcat = if(nodes.size + other.nodes.size <= STAR_MAX_NODES) StarConcatSingle<T>(ctor(nodes + other.nodes, height))
...
inline operator fun <S: Ast, reified T: StarAst<S>> T.plus(other: T): T {
    ...
    val instruction = if(height == other.height) baseCase(other)

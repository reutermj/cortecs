...
@Serializable
data class FunctionAst(override val nodes: List<Ast>, override val errors: CortecsErrors, val nameIndex: Int, val nameSpan: Span, val parametersIndex: Int, val parametersSpan: Span, val returnTypeIndex: Int, val returnTypeSpan: Span, val blockIndex: Int, val blockSpan: Span): TopLevelAst() {
    fun name(): NameToken? = if(nameIndex == -1) null
    else nodes[nameIndex] as NameToken

    fun parameters(): ParametersAst? = if(parametersIndex == -1) null
    else nodes[parametersIndex] as ParametersAst

    fun returnType(): TypeAnnotationToken? = if(returnTypeIndex == -1) null
    else nodes[returnTypeIndex] as TypeAnnotationToken

    fun block(): BlockAst? = if(blockIndex == -1) null
...
@Serializable
data class ParameterAst(override val nodes: List<Ast>, override val errors: CortecsErrors, val nameIndex: Int, val typeAnnotationIndex: Int, val typeAnnotationSpan: Span): AstImpl() {
    fun name(): NameToken = nodes[nameIndex] as NameToken

    fun typeAnnotation(): TypeAnnotationToken? = if(typeAnnotationIndex == -1) null
...
@Serializable
data class LetAst(override val nodes: List<Ast>, override val errors: CortecsErrors, val nameIndex: Int, val nameSpan: Span, val typeAnnotationIndex: Int, val typeAnnotationSpan: Span, val expressionIndex: Int, val expressionSpan: Span): BodyAst() {
...
data class GroupingExpression(override val nodes: List<Ast>, override val errors: CortecsErrors, val expressionIndex: Int, val expressionSpan: Span): BaseExpression() {
    fun expression(): Expression? = if(expressionIndex == -1) null
    else nodes[expressionIndex] as Expression
}
